<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Semi-Finished Goods</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">WIP Batches</div><div class="kpi-val c-b">14</div><div class="kpi-sub">in process</div></div>
      <div class="kpi-card"><div class="kpi-label">WIP Value</div><div class="kpi-val c-d">RM 94,200</div><div class="kpi-sub">work-in-progress stock</div></div>
      <div class="kpi-card"><div class="kpi-label">Awaiting Next Stage</div><div class="kpi-val c-y">6</div><div class="kpi-sub">queued for transfer</div></div>
      <div class="kpi-card"><div class="kpi-label">Hold / Quarantine</div><div class="kpi-val c-r">2</div><div class="kpi-sub">pending QC decision</div></div>
    </div>

    <div class="filter-bar">
      <span class="fl">Status:</span>
      <button class="f-btn" :class="{ on: statusFilter === 'all' }" @click="statusFilter = 'all'">All</button>
      <button class="f-btn" :class="{ on: statusFilter === 'In Process' }" @click="statusFilter = 'In Process'">In Process</button>
      <button class="f-btn" :class="{ on: statusFilter === 'Ready' }" @click="statusFilter = 'Ready'">Ready</button>
      <button class="f-btn" :class="{ on: statusFilter === 'On Hold' }" @click="statusFilter = 'On Hold'">On Hold</button>
      <input class="f-search" type="text" placeholder="Search item…" v-model="search" />
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          WIP Register <span class="ph-r">{{ filteredRows.length }} batches</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>Batch No.</th><th>Item</th><th>Stage</th><th style="text-align:right">Qty</th><th>UOM</th><th>Location</th><th>Status</th></tr>
            </thead>
            <tbody>
              <tr v-for="row in filteredRows" :key="row.batch" class="row-click" @click="openDetail(row)">
                <td>{{ row.batch }}</td>
                <td>{{ row.item }}</td>
                <td>{{ row.stage }}</td>
              ...
              <tr v-if="filteredRows.length === 0"><td colspan="7" class="empty-msg">No batches match the current filter.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div v-if="detail" class="detail-panel">
        <div class="dp-head">
          <div class="dp-head-left">
            <div class="dp-id">{{ detail.batch }}</div>
            <div class="dp-sub">{{ detail.item }}</div>
          </div>
        ...
        <div class="dp-body">
          <div class="dp-grid">
            <div class="dp-field"><span class="dfl">Stage</span><span>{{ detail.stage }}</span></div>
            <div class="dp-field"><span class="dfl">Location</span><span>{{ detail.location }}</span></div>
            <div class="dp-field"><span class="dfl">Quantity</span><span>{{ detail.qty }} {{ detail.uom }}</span></div>
            <div class="dp-field"><span class="dfl">Status</span><span>{{ detail.status }}</span></div>
          </div>
          <div class="sec-title">Process Stage</div>
          <div class="hist-log">
            <div class="hist-row"><span class="hist-dot" :class="statusBadge(detail.status) === 'b-r' ? 'dot-red' : statusBadge(detail.status) === 'b-g' ? 'dot-green' : 'dot-blue'"></span><span class="hist-txt">Currently at: {{ detail.stage }}</span></div>
          </div>
        </div>
        <div class="dp-foot">
          <button class="btn-outline" @click="detail = null">Close</button>
          <button class="btn-primary"><i class="fa-solid fa-arrow-right"></i> Advance Stage</button>
        </div>
      </div>
    </transition>
  </div>
...
interface WipRow { batch: string; item: string; stage: string; qty: string; uom: string; location: string; status: string }

const rows: WipRow[] = [
  { batch: 'WIP-2026-114', item: 'Chili Base Mix',          stage: 'Cooking',     qty: '420',  uom: 'kg', location: 'Batching Plant 01', status: 'In Process' },
  { batch: 'WIP-2026-115', item: 'Curry Base Mix',           stage: 'Blending',    qty: '280',  uom: 'kg', location: 'Batching Plant 02', status: 'In Process' },
  { batch: 'WIP-2026-116', item: 'Sambal Pre-mix',            stage: 'Cooling',     qty: '150',  uom: 'kg', location: 'Cooling Bay',       status: 'In Process' },
  { batch: 'WIP-2026-117', item: 'Rendang Base Mix',          stage: 'QC Sampling', qty: '90',   uom: 'kg', location: 'QC Lab',            status: 'On Hold'    },
  { batch: 'WIP-2026-118', item: 'Tomato Reduction',          stage: 'Cooking',     qty: '310',  uom: 'kg', location: 'Batching Plant 01', status: 'In Process' },
  { batch: 'WIP-2026-119', item: 'Asam Pedas Base Mix',       stage: 'QC Sampling', qty: '60',   uom: 'kg', location: 'QC Lab',            status: 'On Hold'    },
  { batch: 'WIP-2026-120', item: 'Seasoning Pre-blend A',     stage: 'Awaiting FFS', qty: '500', uom: 'kg', location: 'Holding Tank 3',    status: 'Ready'      },
  { batch: 'WIP-2026-121', item: 'Curry Base Mix',            stage: 'Awaiting FFS', qty: '220', uom: 'kg', location: 'Holding Tank 1',    status: 'Ready'      },
]
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    ...
    const searchOk = !q || r.item.toLowerCase().includes(q)
    return statusOk && searchOk
  })
})

function statusBadge(status: string): string {
  if (status === 'Ready')      return 'b-g'
  if (status === 'In Process') return 'b-b'
  if (status === 'On Hold')    return 'b-r'
  return 'b-grey'
}

const detail = ref<WipRow | null>(null)
function openDetail(row: WipRow) { detail.value = row }

async function exportToExcel() {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'HIAS'
  const ws = wb.addWorksheet('Semi-Finished Goods')
  ...
  a.href     = url
  a.download = `Semi_Finished_Goods_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
...
.dot-blue { background: #1565c0; } .dot-green { background: #388E3C; } .dot-red { background: #e53935; }
