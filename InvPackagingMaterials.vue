<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Packaging Materials</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Total Packaging SKUs</div><div class="kpi-val c-b">58</div><div class="kpi-sub">active materials</div></div>
      <div class="kpi-card"><div class="kpi-label">Stock Value</div><div class="kpi-val c-d">RM 168,300</div><div class="kpi-sub">current valuation</div></div>
      <div class="kpi-card"><div class="kpi-label">Low Stock</div><div class="kpi-val c-y">6</div><div class="kpi-sub">below reorder point</div></div>
      <div class="kpi-card"><div class="kpi-label">On Order</div><div class="kpi-val c-g">4</div><div class="kpi-sub">open purchase orders</div></div>
    </div>

    <div class="filter-bar">
      <span class="fl">Status:</span>
      <button class="f-btn" :class="{ on: statusFilter === 'all' }" @click="statusFilter = 'all'">All</button>
      <button class="f-btn" :class="{ on: statusFilter === 'critical' }" @click="statusFilter = 'critical'">Critical</button>
      <button class="f-btn" :class="{ on: statusFilter === 'low' }" @click="statusFilter = 'low'">Low</button>
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Packaging Material Register <span class="ph-r">{{ filteredRows.length }} items shown</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>Code</th><th>Name</th><th>Type</th><th style="text-align:right">On Hand</th><th style="text-align:right">Reorder Pt</th><th>Status</th></tr>
            </thead>
            <tbody>
    ...
    <transition name="panel-slide">
      <div v-if="detail" class="detail-panel">
        ...
        <div class="dp-body">
          <div class="dp-grid">
            <div class="dp-field"><span class="dfl">Type</span><span>{{ detail.type }}</span></div>
...
interface PackRow { code: string; name: string; type: string; onHand: string; reorderPt: string; status: string }

const rows: PackRow[] = [
  { code: 'PM-001', name: 'FFS Film Roll 500mm',         type: 'Flexible Film', onHand: '3,400', reorderPt: '500',   status: 'normal'   },
  { code: 'PM-002', name: 'LDPE Shrink Film 20µm',         type: 'Flexible Film', onHand: '80',    reorderPt: '200',   status: 'critical' },
  { code: 'PM-003', name: 'Corrugated Box 400×300',        type: 'Carton',        onHand: '140',   reorderPt: '200',   status: 'low'       },
  { code: 'PM-004', name: 'Label Sticker 50×30mm',         type: 'Label',         onHand: '1,200', reorderPt: '2,000', status: 'low'       },
  { code: 'PM-005', name: 'PET Bottle 700ml',               type: 'Rigid Bottle',  onHand: '6,200', reorderPt: '1,000', status: 'normal'   },
  { code: 'PM-006', name: 'Tamper Seal Cap 28mm',           type: 'Closure',       onHand: '5,800', reorderPt: '1,500', status: 'normal'   },
  { code: 'PM-007', name: 'Stand-up Pouch 250g',            type: 'Pouch',         onHand: '420',   reorderPt: '800',   status: 'low'       },
]
...
const detail = ref<PackRow | null>(null)
function openDetail(row: PackRow) { detail.value = row }
...
async function exportToExcel() {
  ...
  a.href     = url
  a.download = `Packaging_Materials_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
