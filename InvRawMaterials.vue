<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Raw Materials</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Total RM SKUs</div><div class="kpi-val c-b">132</div><div class="kpi-sub">active materials</div></div>
      <div class="kpi-card"><div class="kpi-label">RM Stock Value</div><div class="kpi-val c-d">RM 387,900</div><div class="kpi-sub">current valuation</div></div>
      <div class="kpi-card"><div class="kpi-label">Low / Critical</div><div class="kpi-val c-r">9</div><div class="kpi-sub">below reorder point</div></div>
      <div class="kpi-card"><div class="kpi-label">Expiring Soon</div><div class="kpi-val c-y">4</div><div class="kpi-sub">within 30 days</div></div>
    </div>

    <div class="filter-bar">
      <span class="fl">Status:</span>
      <button class="f-btn" :class="{ on: status === 'all'      }" @click="status = 'all'">All</button>
      <button class="f-btn" :class="{ on: status === 'lowcrit'  }" @click="status = 'lowcrit'">Low / Critical</button>
      <button class="f-btn" :class="{ on: status === 'expiring' }" @click="status = 'expiring'">Expiring Soon</button>
      <input class="f-search" type="text" placeholder="Search…" v-model="search" />
      <span class="result-count">{{ filteredRows.length }} items shown</span>
    </div>

    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Raw Material Register <span class="ph-r">{{ filteredRows.length }} items shown</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>Code</th><th>Name</th><th>Category</th><th style="text-align:right">On Hand</th><th style="text-align:right">Reorder Pt</th><th>Expiry</th><th>Status</th></tr>
            </thead>
            <tbody>
              ...
              <tr v-if="filteredRows.length === 0"><td colspan="7" class="empty-msg">No items match the current filter.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <transition name="panel-slide">
      <div v-if="detail" class="detail-panel">
        ...
        <div class="dp-body">
          <div class="dp-grid">
            <div class="dp-field"><span class="dfl">Category</span><span>{{ detail.category }}</span></div>
            <div class="dp-field"><span class="dfl">Expiry</span><span>{{ detail.expiry }}</span></div>
            <div class="dp-field"><span class="dfl">On Hand</span><span>{{ detail.onHand }}</span></div>
            <div class="dp-field"><span class="dfl">Reorder Point</span><span>{{ detail.reorderPt }}</span></div>
          </div>
          <div class="sec-title">Status</div>
          <div class="hist-log">
            <div class="hist-row"><span class="hist-dot" :class="statusBadge(detail.status) === 'b-r' ? 'dot-red' : statusBadge(detail.status) === 'b-y' ? 'dot-amber' : 'dot-green'"></span><span class="hist-txt">{{ detail.status }}</span></div>
          </div>
        </div>
        <div class="dp-foot">
          <button class="btn-outline" @click="detail = null">Close</button>
          <button class="btn-primary"><i class="fa-solid fa-cart-shopping"></i> Raise PO</button>
        </div>
      </div>
    </transition>
  </div>
...
const rows = [
  { code: 'RM-001', name: 'Refined Palm Olein (5L)',        category: 'Food Ingredient', onHand: '12',    reorderPt: '50',    expiry: '12/2026', status: 'critical'  },
  { code: 'RM-002', name: 'Tomato Paste Concentrate (Drum)', category: 'Food Ingredient', onHand: '220',   reorderPt: '50',    expiry: '08/2026', status: 'normal'    },
  { code: 'RM-003', name: 'Seasoning Blend A',                category: 'Food Ingredient', onHand: '1,200', reorderPt: '300',   expiry: '03/2027', status: 'normal'    },
  { code: 'RM-004', name: 'Citric Acid E330',                 category: 'Food Additive',   onHand: '28',    reorderPt: '40',    expiry: '06/2027', status: 'low'       },
  { code: 'RM-005', name: 'White Refined Sugar (50kg bag)',   category: 'Food Ingredient', onHand: '85',    reorderPt: '30',    expiry: '—',       status: 'normal'    },
  { code: 'RM-006', name: 'Xanthan Gum E415',                 category: 'Food Additive',   onHand: '5.2',   reorderPt: '10',    expiry: '05/2026', status: 'expiring'  },
  { code: 'RM-007', name: 'Dried Chili Paste (Drum)',          category: 'Food Ingredient', onHand: '40',   reorderPt: '60',    expiry: '05/2026', status: 'expiring'  },
  { code: 'RM-008', name: 'Coconut Milk Powder',               category: 'Food Ingredient', onHand: '95',   reorderPt: '70',    expiry: '06/2026', status: 'expiring'  },
]
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    const statusOk = status.value === 'all'
      || (status.value === 'lowcrit'  && (r.status === 'low' || r.status === 'critical'))
      || (status.value === 'expiring' && r.status === 'expiring')
    const searchOk = !q || (r.code + r.name).toLowerCase().includes(q)
...
const detail = ref<typeof rows[number] | null>(null)
function openDetail(row: typeof rows[number]) { detail.value = row }
...
async function exportToExcel() {
  ...
  ws.columns = [
    ...
    { header: 'Reorder Pt', key: 'reorderPt',  width: 12 },
  ...
  a.href     = url
  a.download = `Raw_Materials_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
