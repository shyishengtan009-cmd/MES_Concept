<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Sample Retention Management</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Samples in Storage</div><div class="kpi-val c-b">214</div><div class="kpi-sub">active retention samples</div></div>
      <div class="kpi-card"><div class="kpi-label">Due for Disposal</div><div class="kpi-val c-y">12</div><div class="kpi-sub">within 14 days</div></div>
      <div class="kpi-card"><div class="kpi-label">Storage Locations</div><div class="kpi-val c-d">3</div><div class="kpi-sub">retention rooms</div></div>
      <div class="kpi-card"><div class="kpi-label">Overdue</div><div class="kpi-val c-r">2</div><div class="kpi-sub">past retention period</div></div>
    </div>

    <div class="filter-bar">
      <span class="fl">Status:</span>
      <button class="f-btn" :class="{ on: statusFilter === 'all' }" @click="statusFilter = 'all'">All</button>
      <button class="f-btn" :class="{ on: statusFilter === 'Stored' }" @click="statusFilter = 'Stored'">Stored</button>
      ...
      <input class="f-search" type="text" placeholder="Search product / sample…" v-model="search" />
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Retention Sample Register <span class="ph-r">{{ filteredRows.length }} samples</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>Sample ID</th><th>Product</th><th>Batch No.</th><th>Retention Until</th><th>Location</th><th>Status</th></tr>
            </thead>
            <tbody>
              <tr v-for="row in filteredRows" :key="row.sampleId" class="row-click" @click="openDetail(row)">
                <td>{{ row.sampleId }}</td>
                <td>{{ row.product }}</td>
                <td>{{ row.batch }}</td>
              ...
              <tr v-if="filteredRows.length === 0"><td colspan="6" class="empty-msg">No samples match the current filter.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div v-if="detail" class="detail-panel">
        <div class="dp-head">
          <div class="dp-head-left">
            <div class="dp-id">{{ detail.sampleId }}</div>
        ...
        <div class="dp-body">
          <div class="dp-grid">
            ...
            <div class="dp-field"><span class="dfl">Retention Until</span><span>{{ detail.retentionUntil }}</span></div>
        ...
        <div class="dp-foot">
          ...
          <button class="btn-primary"><i class="fa-solid fa-trash"></i> Dispose Sample</button>
        </div>
      </div>
    </transition>
  </div>
...
// Current-batch samples resolve product names via the shared catalog (stays in sync
// with Finished Goods / BMR). Older retained batches predate the catalog's latest
// batch and keep their own historical product label.
const SAMPLE_ROWS = [
  { sampleId: 'RS-2026-0411', batch: 'B2026-0411', product: null,                          retentionUntil: '11 Apr 2027', location: 'Retention Room A', status: 'Stored'   },
  { sampleId: 'RS-2026-0409', batch: 'B2026-0409', product: null,                          retentionUntil: '09 Apr 2027', location: 'Retention Room A', status: 'Stored'   },
  { sampleId: 'RS-2026-0412', batch: 'B2026-0412', product: null,                          retentionUntil: '12 Apr 2027', location: 'Retention Room A', status: 'Stored'   },
  { sampleId: 'RS-2025-0118', batch: 'B2025-0118', product: 'Sambal Belacan Jar 200g',     retentionUntil: '18 Apr 2026', location: 'Retention Room B', status: 'Due Soon' },
  { sampleId: 'RS-2025-0102', batch: 'B2025-0102', product: 'Seasoning Blend A Sachet',    retentionUntil: '02 Apr 2026', location: 'Retention Room B', status: 'Overdue'  },
  { sampleId: 'RS-2025-0096', batch: 'B2025-0096', product: 'Tomato Chili Sauce Bottle',   retentionUntil: '20 Mar 2026', location: 'Retention Room C', status: 'Overdue'  },
]

const rows = SAMPLE_ROWS.map(r => ({ ...r, product: r.product ?? findByBatch(r.batch)?.name ?? '—' }))
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    ...
    const searchOk = !q || (r.sampleId + r.product).toLowerCase().includes(q)
    return statusOk && searchOk
  })
})

function statusBadge(status: string): string {
  if (status === 'Stored')   return 'b-g'
  if (status === 'Due Soon') return 'b-y'
  if (status === 'Overdue')  return 'b-r'
  return 'b-grey'
}

const detail = ref<SampleRow | null>(null)
function openDetail(row: SampleRow) { detail.value = row }

async function exportToExcel() {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'HIAS'
  const ws = wb.addWorksheet('Sample Retention')

  ws.columns = [
    { header: 'Sample ID',        key: 'sampleId',       width: 16 },
    { header: 'Product',          key: 'product',        width: 26 },
    { header: 'Batch No.',        key: 'batch',           width: 14 },
    { header: 'Retention Until',  key: 'retentionUntil',  width: 16 },
  ...
  filteredRows.value.forEach((r, i) => {
    ...
    if (r.status === 'Stored')   { statusCell.font = { ...statusCell.font as ExcelJS.Font, color: { argb: 'FF2E7D32' }, bold: true } }
  ...
  a.href     = url
  a.download = `Sample_Retention_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
