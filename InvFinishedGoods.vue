<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Finished Goods / Products</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Total Finished SKUs</div><div class="kpi-val c-b">86</div><div class="kpi-sub">active products</div></div>
      <div class="kpi-card"><div class="kpi-label">Stock Value</div><div class="kpi-val c-d">RM 612,400</div><div class="kpi-sub">finished goods on hand</div></div>
      <div class="kpi-card"><div class="kpi-label">Low Stock</div><div class="kpi-val c-y">5</div><div class="kpi-sub">below reorder point</div></div>
      <div class="kpi-card"><div class="kpi-label">Pending QC Release</div><div class="kpi-val c-r">3</div><div class="kpi-sub">held for inspection</div></div>
    </div>

    <div class="filter-bar">
      <span class="fl">Category:</span>
      <button class="f-btn" :class="{ on: category === 'all'    }" @click="category = 'all'">All</button>
      <button class="f-btn" :class="{ on: category === 'sauce'  }" @click="category = 'sauce'">Sauces</button>
      <button class="f-btn" :class="{ on: category === 'paste'  }" @click="category = 'paste'">Pastes</button>
      <button class="f-btn" :class="{ on: category === 'season' }" @click="category = 'season'">Seasonings</button>
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Finished Goods Register <span class="ph-r">{{ filteredRows.length }} items shown</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>Code</th><th>Product Name</th><th>Category</th><th>Batch No.</th><th style="text-align:right">On Hand</th><th>UOM</th><th>Status</th></tr>
            </thead>
            <tbody>
    ...
    <transition name="panel-slide">
      <div v-if="detail" class="detail-panel">
        ...
        <div class="dp-body">
          ...
          <div class="sec-title">Traceability</div>
          <div class="hist-log">
            <div class="hist-row"><span class="hist-dot dot-blue"></span><span class="hist-txt">Batch {{ detail.batch }} — see Traceability tab for full chain</span></div>
          </div>
        </div>
        <div class="dp-foot">
          <button class="btn-outline" @click="detail = null">Close</button>
          <button class="btn-primary"><i class="fa-solid fa-tag"></i> Generate Label</button>
        </div>
      </div>
    </transition>
  </div>
...
import { FINISHED_GOODS } from './src/inventoryCatalog'
...
// On-hand qty / status are register-specific, layered on top of the shared catalog
const STOCK_BY_CODE: Record<string, { onHand: string; status: string }> = {
  'FG-001': { onHand: '8,400', status: 'Released'  },
  'FG-002': { onHand: '5,200', status: 'Released'  },
  'FG-003': { onHand: '2,100', status: 'Released'  },
  'FG-004': { onHand: '480',   status: 'QC Hold'   },
  'FG-005': { onHand: '1,150', status: 'Low Stock' },
  'FG-006': { onHand: '3,900', status: 'Released'  },
  'FG-007': { onHand: '210',   status: 'QC Hold'   },
  'FG-008': { onHand: '760',   status: 'Low Stock' },
}

const rows = FINISHED_GOODS.map(p => ({
  code: p.code, name: p.name, category: p.category, batch: p.latestBatch, uom: p.uom,
  ...STOCK_BY_CODE[p.code],
}))

type FgRow = typeof rows[number]
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    const catOk = category.value === 'all' || r.category === category.value
...
function statusBadge(status: string): string {
  if (status === 'Released')  return 'b-g'
  if (status === 'Low Stock') return 'b-y'
  if (status === 'QC Hold')   return 'b-r'
  return 'b-grey'
}

const detail = ref<FgRow | null>(null)
function openDetail(row: FgRow) { detail.value = row }

async function exportToExcel() {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'HIAS'
  const ws = wb.addWorksheet('Finished Goods')
  ...
  filteredRows.value.forEach((r, i) => {
    ...
    if (r.status === 'Released')  { statusCell.font = { ...statusCell.font as ExcelJS.Font, color: { argb: 'FF2E7D32' }, bold: true } }
    if (r.status === 'Low Stock') { statusCell.font = { ...statusCell.font as ExcelJS.Font, color: { argb: 'FFE65100' }, bold: true } }
    if (r.status === 'QC Hold')   { statusCell.font = { ...statusCell.font as ExcelJS.Font, color: { argb: 'FFC62828' }, bold: true } }
  })
  ...
  a.href     = url
  a.download = `Finished_Goods_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
