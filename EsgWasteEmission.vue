<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Waste & Emission Tracking</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Waste Generated MTD</div><div class="kpi-val c-b">18.4 t</div><div class="kpi-sub">all waste streams</div></div>
      <div class="kpi-card"><div class="kpi-label">Recycled / Diverted</div><div class="kpi-val c-g">62%</div><div class="kpi-sub">of total waste</div></div>
      <div class="kpi-card"><div class="kpi-label">CO2 Emissions MTD</div><div class="kpi-val c-y">142 t</div><div class="kpi-sub">scope 1 &amp; 2</div></div>
      <div class="kpi-card"><div class="kpi-label">Hazardous Waste</div><div class="kpi-val c-r">0.6 t</div><div class="kpi-sub">licensed disposal</div></div>
    </div>
    ...
    <div class="filter-bar">
      ...
      <input class="f-search" type="text" placeholder="Search method…" v-model="search" />
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Waste Disposal Log <span class="ph-r">{{ filteredRows.length }} records</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead><tr><th>Date</th><th>Waste Type</th><th style="text-align:right">Qty</th><th>Disposal Method</th><th>Status</th></tr></thead>
            <tbody>
              <tr v-for="(row, i) in filteredRows" :key="i" class="row-click" @click="openDetail(row)">
                <td>{{ row.date }}</td><td>{{ row.type }}</td><td style="text-align:right">{{ row.qty }}</td>
                <td>{{ row.method }}</td>
                <td><span class="badge" :class="row.status === 'Completed' ? 'b-g' : 'b-y'">{{ row.status }}</span></td>
              </tr>
              <tr v-if="filteredRows.length === 0"><td colspan="5" class="empty-msg">No records match the current filter.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div v-if="detail" class="detail-panel">
        <div class="dp-head">
          <div class="dp-head-left">
            ...
            <div class="dp-sub">{{ detail.date }}</div>
          </div>
          <span class="badge" :class="detail.status === 'Completed' ? 'b-g' : 'b-y'">{{ detail.status }}</span>
        ...
        <div class="dp-body">
          <div class="dp-grid">
            ...
            <div class="dp-field"><span class="dfl">Disposal Method</span><span>{{ detail.method }}</span></div>
            <div class="dp-field"><span class="dfl">Status</span><span>{{ detail.status }}</span></div>
          </div>
          <div class="sec-title">Disposal Trail</div>
          <div class="hist-log">
            <div v-for="(h, i) in detailHistory" :key="i" class="hist-row">
              <span class="hist-dot" :class="h.dot"></span>
              <span class="hist-date">{{ h.text }}</span>
            </div>
          </div>
        </div>
        <div class="dp-foot">
          <button class="btn-outline" @click="detail = null">Close</button>
          <button class="btn-primary"><i class="fa-solid fa-file-pdf"></i> Waste Manifest</button>
        </div>
      </div>
    </transition>
  </div>
...
interface WasteRow { date: string; type: string; qty: string; method: string; status: string }

const rows: WasteRow[] = [
  { date: '12 Apr 2026', type: 'Organic Food Waste',  qty: '2.1 t',  method: 'Composting',        status: 'Completed' },
  { date: '11 Apr 2026', type: 'Plastic Packaging',    qty: '0.8 t',  method: 'Recycling Facility', status: 'Completed' },
  { date: '10 Apr 2026', type: 'Cardboard / Carton',   qty: '1.4 t',  method: 'Recycling Facility', status: 'Completed' },
  { date: '09 Apr 2026', type: 'Used Lubricant Oil',   qty: '0.3 t',  method: 'Licensed Contractor', status: 'Scheduled' },
  { date: '08 Apr 2026', type: 'Wastewater Sludge',    qty: '1.0 t',  method: 'Treatment Plant',    status: 'Completed' },
]

const types = ['Organic Food Waste', 'Plastic Packaging', 'Cardboard / Carton', 'Used Lubricant Oil', 'Wastewater Sludge']
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    ...
    const searchOk = !q || r.method.toLowerCase().includes(q)
    return typeOk && searchOk
  })
})

const detail = ref<WasteRow | null>(null)
const detailHistory = ref<{ text: string; dot: string }[]>([])

function openDetail(row: WasteRow) {
  detail.value = row
  detailHistory.value = [
    { text: `Collected — ${row.qty}`, dot: 'dot-blue' },
    { text: `Transported to ${row.method}`, dot: 'dot-amber' },
    { text: row.status === 'Completed' ? 'Disposal confirmed' : 'Awaiting collection', dot: row.status === 'Completed' ? 'dot-green' : 'dot-amber' },
  ]
}
...
async function exportToExcel() {
  ...
  wb.creator = 'HIAS'
  const ws = wb.addWorksheet('Waste Disposal Log')

  ws.columns = [
    { header: 'Date',             key: 'date',   width: 14 },
    { header: 'Waste Type',       key: 'type',   width: 22 },
    { header: 'Qty',              key: 'qty',    width: 10 },
    { header: 'Disposal Method',  key: 'method', width: 22 },
  ...
  a.href     = url
  a.download = `Waste_Emission_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
...
.hist-date { color: #515151; font-size: 11px; }
