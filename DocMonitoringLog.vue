<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Monitoring Log & Report</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Logs Today</div><div class="kpi-val c-b">42</div><div class="kpi-sub">readings recorded</div></div>
      <div class="kpi-card"><div class="kpi-label">Logs This Week</div><div class="kpi-val c-d">286</div><div class="kpi-sub">total entries</div></div>
      <div class="kpi-card"><div class="kpi-label">Deviations Flagged</div><div class="kpi-val c-r">3</div><div class="kpi-sub">out of spec</div></div>
      <div class="kpi-card"><div class="kpi-label">Reports Pending</div><div class="kpi-val c-y">2</div><div class="kpi-sub">awaiting sign-off</div></div>
    </div>
    ...
    <div class="filter-bar">
      ...
      <button class="f-btn" :class="{ on: statusFilter === 'Normal' }" @click="statusFilter = 'Normal'">Normal</button>
      <button class="f-btn" :class="{ on: statusFilter === 'Deviation' }" @click="statusFilter = 'Deviation'">Deviation</button>
      <input class="f-search" type="text" placeholder="Search area…" v-model="search" />
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Monitoring Log <span class="ph-r">{{ filteredRows.length }} entries</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead><tr><th>Date / Time</th><th>Log Type</th><th>Area</th><th>Recorded By</th><th>Reading</th><th>Status</th></tr></thead>
            <tbody>
              <tr v-for="(row, i) in filteredRows" :key="i" class="row-click" @click="openDetail(row)">
                <td>{{ row.time }}</td><td>{{ row.type }}</td><td>{{ row.area }}</td>
                <td>{{ row.by }}</td><td>{{ row.reading }}</td>
                <td><span class="badge" :class="row.status === 'Normal' ? 'b-g' : 'b-r'">{{ row.status }}</span></td>
              </tr>
              <tr v-if="filteredRows.length === 0"><td colspan="6" class="empty-msg">No entries match the current filter.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div v-if="detail" class="detail-panel">
        <div class="dp-head">
          <div class="dp-head-left">
            <div class="dp-id">{{ detail.type }}</div>
            <div class="dp-sub">{{ detail.area }} &mdash; {{ detail.time }}</div>
          </div>
          <span class="badge" :class="detail.status === 'Normal' ? 'b-g' : 'b-r'">{{ detail.status }}</span>
          <button class="dp-close" @click="detail = null"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div class="dp-body">
          <div class="dp-grid">
            <div class="dp-field"><span class="dfl">Area</span><span>{{ detail.area }}</span></div>
            <div class="dp-field"><span class="dfl">Recorded By</span><span>{{ detail.by }}</span></div>
            <div class="dp-field"><span class="dfl">Reading</span><span>{{ detail.reading }}</span></div>
            <div class="dp-field"><span class="dfl">Status</span><span>{{ detail.status }}</span></div>
          </div>
          <div class="sec-title">Recent Readings &mdash; {{ detail.area }}</div>
          <div class="hist-log">
            <div v-for="(h, i) in detailHistory" :key="i" class="hist-row">
              <span class="hist-dot" :class="h.dot"></span>
              <span class="hist-date">{{ h.time }}</span>
        ...
        <div class="dp-foot">
          ...
          <button class="btn-primary"><i class="fa-solid fa-flag"></i> Flag for Review</button>
        </div>
      </div>
    </transition>
  </div>
...
interface LogRow { time: string; type: string; area: string; by: string; reading: string; status: string }

const rows: LogRow[] = [
  { time: '10:30', type: 'Cold Room Temp',     area: 'Cold Storage 1', by: 'Saliza', reading: '4.2 °C',  status: 'Normal' },
  { time: '09:30', type: 'Chiller Temp',        area: 'Cold Storage 2', by: 'Amirul', reading: '8.6 °C',  status: 'Deviation' },
  { time: '08:30', type: 'Sanitation Check',     area: 'FFS Packaging Line', by: 'Farah', reading: 'Pass', status: 'Normal' },
  { time: '07:30', type: 'pH Reading',           area: 'QC Lab',          by: 'QA – Hafiz', reading: '4.1',  status: 'Normal' },
  { time: 'Yest 22:00', type: 'Cold Room Temp', area: 'Cold Storage 1', by: 'Night Shift', reading: '5.0 °C', status: 'Normal' },
  { time: 'Yest 18:00', type: 'Water Activity',  area: 'QC Lab',          by: 'QA – Mei Ling', reading: '0.62', status: 'Deviation' },
]

const types = ['Cold Room Temp', 'Chiller Temp', 'Sanitation Check', 'pH Reading', 'Water Activity']
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    ...
    const searchOk = !q || r.area.toLowerCase().includes(q)
    return typeOk && statusOk && searchOk
  })
})

const detail = ref<LogRow | null>(null)
const detailHistory = ref<{ time: string; text: string; dot: string }[]>([])

function openDetail(row: LogRow) {
  detail.value = row
  detailHistory.value = [
    { time: row.time, text: `Reading ${row.reading} recorded by ${row.by}`, dot: row.status === 'Normal' ? 'dot-green' : 'dot-red' },
    { time: '6h prior', text: 'Reading within normal range', dot: 'dot-green' },
    { time: '12h prior', text: 'Reading within normal range', dot: 'dot-green' },
  ]
}
...
async function exportToExcel() {
  ...
  wb.creator = 'HIAS'
  const ws = wb.addWorksheet('Monitoring Log')

  ws.columns = [
    { header: 'Date / Time',  key: 'time',    width: 14 },
    { header: 'Log Type',     key: 'type',    width: 18 },
    { header: 'Area',         key: 'area',    width: 20 },
    { header: 'Recorded By',  key: 'by',      width: 16 },
  ...
  filteredRows.value.forEach((r, i) => {
    ...
    statusCell.font = { ...statusCell.font as ExcelJS.Font, color: { argb: r.status === 'Normal' ? 'FF2E7D32' : 'FFC62828' }, bold: true }
  })
  ...
  a.href     = url
  a.download = `Monitoring_Log_${new Date().toISOString().slice(0, 10)}.xlsx`
  a.click()
...
.dot-green { background: #388E3C; } .dot-red { background: #e53935; }
