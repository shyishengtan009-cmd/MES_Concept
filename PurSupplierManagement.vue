<template>
  <div class="pg-root">
    <div class="topbar">
      <span class="pg-title">&#9632; Supplier Management</span>
      <div class="topbar-right"><span class="live-dot"></span><span>{{ clock }}</span></div>
    </div>

    <div class="kpi-row">
      <div class="kpi-card"><div class="kpi-label">Total Suppliers</div><div class="kpi-val c-b">38</div><div class="kpi-sub">registered vendors</div></div>
      <div class="kpi-card"><div class="kpi-label">Active</div><div class="kpi-val c-g">34</div><div class="kpi-sub">approved &amp; trading</div></div>
      <div class="kpi-card"><div class="kpi-label">Pending Approval</div><div class="kpi-val c-y">4</div><div class="kpi-sub">awaiting audit</div></div>
      <div class="kpi-card"><div class="kpi-label">Avg Lead Time</div><div class="kpi-val c-d">6.2 days</div><div class="kpi-sub">across all suppliers</div></div>
    </div>

    <div class="filter-bar">
      <span class="fl">Category:</span>
      <button class="f-btn" :class="{ on: categoryFilter === 'all' }" @click="categoryFilter = 'all'">All</button>
      <button v-for="c in categories" :key="c" class="f-btn" :class="{ on: categoryFilter === c }" @click="categoryFilter = c">{{ c }}</button>
      ...
      <input class="f-search" type="text" placeholder="Search supplier…" v-model="search" />
    ...
    <div class="body-full">
      <div class="pc" style="flex:1">
        <div class="ph">
          Supplier Register <span class="ph-r">{{ filteredRows.length }} suppliers</span>
          <button class="export-btn" @click="exportToExcel"><i class="fa-solid fa-arrow-up-from-bracket"></i> Export</button>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead><tr><th>Supplier Code</th><th>Name</th><th>Category</th><th>Rating</th><th>Status</th></tr></thead>
            <tbody>
              <tr v-for="row in filteredRows" :key="row.code" class="row-click" @click="openDetail(row)">
                <td>{{ row.code }}</td><td>{{ row.name }}</td><td>{{ row.category }}</td>
                <td>{{ row.rating }}</td>
                <td><span class="badge" :class="statusBadge(row.status)">{{ row.status }}</span></td>
              </tr>
              <tr v-if="filteredRows.length === 0"><td colspan="5" class="empty-msg">No suppliers match the current filter.</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div v-if="detail" class="detail-panel">
        ...
        <div class="dp-body">
          <div class="dp-grid">
            ...
            <div class="dp-field"><span class="dfl">Rating</span><span>{{ detail.rating }}</span></div>
            <div class="dp-field"><span class="dfl">Status</span><span>{{ detail.status }}</span></div>
          </div>
          <div class="sec-title">Recent Purchase Orders</div>
          <div class="hist-log">
        ...
        <div class="dp-foot">
          ...
          <button class="btn-primary"><i class="fa-solid fa-file-invoice"></i> New PO</button>
        </div>
      </div>
    </transition>
  </div>
...
interface SupplierRow { code: string; name: string; category: string; rating: string; status: string }

const rows: SupplierRow[] = [
  { code: 'SUP-001', name: 'Borneo Oils Trading',         category: 'Food Ingredient', rating: '★★★★☆', status: 'Active'  },
  { code: 'SUP-002', name: 'Golden Grain Imports',         category: 'Food Ingredient', rating: '★★★☆☆', status: 'Active'  },
  { code: 'SUP-003', name: 'PolyPack Industries',          category: 'Packaging',       rating: '★★★★★', status: 'Active'  },
  { code: 'SUP-004', name: 'Sinar Pacific Distribution',   category: 'Trading',         rating: '★★★★☆', status: 'Active'  },
  { code: 'SUP-005', name: 'ColdChain Asia',                category: 'Logistics',       rating: '★★★☆☆', status: 'Pending' },
  { code: 'SUP-006', name: 'Spice Valley Sdn Bhd',          category: 'Food Ingredient', rating: '★★★★☆', status: 'Pending' },
]

const categories = ['Food Ingredient', 'Packaging', 'Trading', 'Logistics']
...
const filteredRows = computed(() => {
  ...
  return rows.filter(r => {
    const catOk    = categoryFilter.value === 'all' || r.category === categoryFilter.value
...
function openDetail(row: SupplierRow) {
  detail.value = row
  detailHistory.value = [
    { date: '11 Apr 2026', text: 'PO-2026-0211 — delivered on time', dot: 'dot-green' },
    { date: '28 Mar 2026', text: 'PO-2026-0198 — delivered on time', dot: 'dot-green' },
    { date: '14 Mar 2026', text: 'PO-2026-0182 — 1 day late',        dot: 'dot-amber' },
  ]
}
...
.dot-green { background: #388E3C; } .dot-amber { background: #f9a825; }
